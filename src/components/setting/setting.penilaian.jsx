...
export function SettingPenilaian({ matrik, userLogin }) {
  ...
  const divisiLeader = divisi.find(
    (d) => d.leaderId === userLogin.id
  )?.karyawan;
  ...
  const handleCreate = async (id) => {
    try {
      if (divisiLeader?.penilaian) {
        toast.error("Penilaian sudah dibuat");
      } else {
      ...
      {divisiLeader.map((d) => (
        ...
        >
          <div className="w-full flex justify-between">
            <div className="w-full text-2xl p-2 font-semibold">{d.nama}</div>
            {d?.penilaian.length >= 1 && (
              <div
                onClick={() => toggleDropdown(d.id)}
                className="w-10 aspect-square p-2 top-[20%] cursor-pointer "
              >
                <ChevronsDown
                  size={20}
